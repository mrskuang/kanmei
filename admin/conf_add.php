    <?php 
        ...
        //添加操作
        if(isset($_POST['add'])){
            $name=$_POST['conf_copy'];
       
            $sql="INSERT INTO km_conf (`conf_copy`) VALUES('$name')";
            $res=insert($sql);
            if($res['code']==1){
        ...
        if(isset($_GET['edit_id'])){
            $id=$_GET['edit_id'];
            $sql="SELECT * FROM km_conf WHERE conf_id = $id";
            $findinfo=find($sql);
        ...
        if(isset($_POST['edit'])){
            ...
            $sql="UPDATE km_conf SET `conf_copy` = '$name' WHERE `conf_id` = {$id}";
                    ...
                    <input type="hidden" name="conf_id" value="<?php  if(isset($findinfo)){ echo $findinfo['conf_id'];}else{ echo ""; }?>" />
                        <div class="panel">
                            <div class="panel-heading">
                                <div class="panel-title"> <?php if(isset($findinfo)){ echo "修改网站配置信息";}else{echo "添加网站配置信息";} ?></div>
                                <div class="panel-btns pull-right margin-left">
                                    <a href="conf_list.php"
